"""Kabul Jobs - local job aggregator / board (plausible URL; adjust if site differs)."""
import hashlib
import requests
...
LOG = get_logger("kabul_jobs")

# If the actual site URL differs, set KABUL_JOBS_BASE in .env or change here
BASE_URL = "https://www.kabuljobs.com"
JOBS_PATHS = ["/jobs", "/vacancies", "/", ""]


def _discover_from_url(session: requests.Session, url: str, jobs: list, seen_urls: set, adapter: "KabulJobsAdapter") -> None:
    try:
        ...
        for a in soup.select('a[href*="job"], a[href*="vacanc"], a[href*="career"], [class*="job"] a'):
            ...
            if not title or len(title) < 5:
                continue
            job_id = "kabuljobs_" + hashlib.sha256(href.encode()).hexdigest()[:14]
            listing = JobListing(
                id=job_id,
                title=title,
                company="",
        ...
        LOG.warning("kabul_jobs discover from %s failed: %s", url, e)


class KabulJobsAdapter(SiteAdapter):
    name = "kabul_jobs"
