"""CTG (Humanitarian) - ctg.org work-with-us jobs."""
import hashlib
import requests
...
LOG = get_logger("ctg_global")

BASE_URL = "https://ctg.org"
JOBS_URL = "https://ctg.org/work-with-us/jobs/"


class CtgGlobalAdapter(SiteAdapter):
    name = "ctg_global"
    ...
    def discover_jobs(self) -> List[JobListing]:
        ...
        try:
            ...
            seen = set()
            for a in soup.select('a[href*="job"], a[href*="career"], a[href*="vacanc"], [class*="job"] a, [class*="listing"] a'):
                href = a.get("href", "")
                if not href.startswith("http"):
                    href = BASE_URL.rstrip("/") + "/" + href.lstrip("/")
                if href in seen or "ctg.org" not in href:
                    continue
                seen.add(href)
                title = (a.get_text(strip=True) or "").strip()
                if not title or len(title) < 5:
                    continue
                job_id = "ctg_" + hashlib.sha256(href.encode()).hexdigest()[:14]
                listing = JobListing(
                    id=job_id,
                    title=title,
                    company="CTG Global",
            ...
            LOG.warning("  [ctg_global] Discovery error: %s", e)
        return jobs
    ...
    def apply(self, job: JobListing, cv_path: str, cover_letter_path: Optional[str] = None) -> bool:
        return apply_via_browser(
            ...
            skip_domains=["ctg.org"], adapter_name=self.name,
        )
