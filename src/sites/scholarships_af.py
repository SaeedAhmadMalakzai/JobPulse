"""Scholarships.af - opportunities (jobs and vacancies section)."""
import hashlib
import requests
...
LOG = get_logger("scholarships_af")

BASE_URL = "https://scholarships.af"
OPPORTUNITIES_URL = "https://scholarships.af/opportunity/"


class ScholarshipsAfAdapter(SiteAdapter):
    name = "scholarships_af"

    def discover_jobs(self) -> List[JobListing]:
        jobs = []
        try:
            headers = {"User-Agent": "Mozilla/5.0 (compatible; CV-Bot/1.0)"}
            resp = requests.get(OPPORTUNITIES_URL, headers=headers, timeout=30)
            resp.raise_for_status()
            soup = BeautifulSoup(resp.text, "lxml")
            seen = set()
            for a in soup.select('a[href*="/opportunity/"]'):
                href = a.get("href", "")
                if not href.startswith("http"):
                    href = BASE_URL.rstrip("/") + "/" + href.lstrip("/")
                if href in seen or "/opportunity/" not in href:
                    continue
                seen.add(href)
                title = (a.get_text(strip=True) or "").strip()
                if not title or len(title) < 5:
                    continue
                job_id = "scholarships_af_" + hashlib.sha256(href.encode()).hexdigest()[:12]
                listing = JobListing(
                    id=job_id,
                    title=title,
                    company="",
            ...
            LOG.warning("scholarships_af discover_jobs failed: %s", e)
        return jobs
