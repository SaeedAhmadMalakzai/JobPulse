"""Afghanistan NGO Jobs - NGO job board (plausible URL; adjust if site differs)."""
import hashlib
import requests
...
LOG = get_logger("afghanistan_ngo_jobs")

BASE_URL = "https://www.afghanistanngojobs.org"
JOBS_PATHS = ["/jobs", "/vacancies", "/opportunities", "/", ""]


def _discover_from_url(session: requests.Session, url: str, jobs: list, seen_urls: set, adapter: "AfghanistanNgoJobsAdapter") -> None:
    try:
        r = session.get(url, timeout=25)
        r.raise_for_status()
        soup = BeautifulSoup(r.text, "lxml")
        for a in soup.select('a[href*="job"], a[href*="vacanc"], a[href*="opportunit"], [class*="job"] a'):
            href = a.get("href", "")
            ...
            if not title or len(title) < 5:
                continue
            job_id = "afgngojobs_" + hashlib.sha256(href.encode()).hexdigest()[:12]
            listing = JobListing(
                id=job_id,
                title=title,
                company="",
        ...
        LOG.warning("afghanistan_ngo_jobs discover from %s failed: %s", url, e)


class AfghanistanNgoJobsAdapter(SiteAdapter):
    name = "afghanistan_ngo_jobs"
    ...
    def apply(self, job: JobListing, cv_path: str, cover_letter_path: Optional[str] = None) -> bool:
        return apply_via_browser(
            ...
            cv_path,
            cover_letter_path,
            skip_domains=["afghanistanngojobs.org"],
