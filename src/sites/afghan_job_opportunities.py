"""Afghan Job Opportunities - job aggregator (plausible URL; adjust if site differs)."""
import hashlib
import requests
...
LOG = get_logger("afghan_job_opportunities")

BASE_URL = "https://www.afghanjobopportunities.com"
JOBS_PATHS = ["/jobs", "/vacancies", "/", ""]


def _discover_from_url(session: requests.Session, url: str, jobs: list, seen_urls: set, adapter: "AfghanJobOpportunitiesAdapter") -> None:
    try:
        ...
        for a in soup.select('a[href*="job"], a[href*="vacanc"], a[href*="opportunit"], [class*="job"] a'):
            ...
            if not title or len(title) < 5:
                continue
            job_id = "afgjobopp_" + hashlib.sha256(href.encode()).hexdigest()[:12]
            listing = JobListing(
                id=job_id,
                title=title,
                company="",
        ...
        LOG.warning("afghan_job_opportunities discover from %s failed: %s", url, e)


class AfghanJobOpportunitiesAdapter(SiteAdapter):
    name = "afghan_job_opportunities"
    ...
    def apply(self, job: JobListing, cv_path: str, cover_letter_path: Optional[str] = None) -> bool:
        return apply_via_browser(
            ...
            cv_path,
            cover_letter_path,
            skip_domains=["afghanjobopportunities.com"],
